...
def write_and_compile_latex_PDF(T, W, P, sys, Mpi):
	...
	density_matrix_plot(P, T, K, sys.bands)
...
def dipole_quiver_plots(K, P, sys):
	...
	d_x = np.zeros([Nk_combined, sys.bands, sys.bands], dtype=np.complex128)
	d_y = np.zeros([Nk_combined, sys.bands, sys.bands], dtype=np.complex128)
...
def density_matrix_plot(P, T, K, bands):
	i_band, j_band = 1, 1
	reshaped_pdf_dm = np.zeros((P.Nk1*P.Nk2, P.Nt_pdf_densmat, bands, bands), dtype=P.type_complex_np)
	...
	replace("bandindex in {0,...,1}", "bandindex in {0,...," + str(bands-1) + "}")
